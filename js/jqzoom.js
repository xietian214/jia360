;(function($){
	$.fn.zoom = function(options){
		...
		var _option = {
			...
			thumb_image_width: 438,		// 当前展示图片的宽
			thumb_image_height: 438,	// 当前展示图片的高
			source_image_width: 896,  	// 放大图片的宽
			source_image_height: 896,	// 放大图片的高
			zoom_area_width: 896, 		// 放大图片的展示区域的宽
		...
		if($ul.is("ul") && $ul.children("li").length && $ul.find(".etalage_big_image").length){
			...
			$li.add($etalage_magnifier).mousemove(function(event){
				...
				autoPlay.stop();
				var _src = $('#etalage>li').eq($small.find('li.etalage_smallthumb_active').index()).find('.etalage_big_image').attr('src');
				$etalage_zoom_img.attr("src", _src);
